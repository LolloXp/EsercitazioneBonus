...
public:
	...
	/// @brief to set dimensions of the Rhombus
	void SetHorD(float dH);
	void SetVerD(float dV);
	void SetDim(float dH, float dV);

	/// @brief to get dimensions of the Rhombus
	float GetHorD();
	float GetVerD();
	void GetDim(float& dH, float& dV);
	float GetSide();

	/// @brief to calculate informations about the Rhombus
	float GetArea();
	float GetPerimeter();
	int Check();
	/// @brief per stampare i dati
	void Dump();
	void Draw();
};
